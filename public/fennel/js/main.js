...
function checkKey(e) {
	...
	var slider,
		...
		d3.csv("http://invasivericochet.com/fennel/data/fennel.csv",
			type,
			function(error, data) {
	      ...
	      .on("click",function(){
			...
			timescale
				...
				.domain([new Date(minyr - 1,0,1),new Date(maxyr - 1,12,31)]);
			...
			d3.select(".graphindicator") // line that goes down to the chart
					...
					xScale = d3.scale.linear().range([100, width - 100]).domain([minyr - 1,maxyr - 1]),	
					yScale = d3.scale.linear().range([180, 20]).domain([0,2800]),
	      		...
	      		.data(taxon)
					  ...
					  d3.select("#show-voronoi")
						...
						function mouseout(d) {
							...
							// load story data
							d3.csv("http://invasivericochet.com/fennel/fennels.csv", function(msg) {
									...
									d3.csv("http://invasivericochet.com/fennel/data/fennelstorylabels.csv", function(lab) {
							...
							update = function(data,gate) {
										...
										var observations = usedat.length;
										var doctitle = document.title;
										
										var socket = io();
										
										socket.emit('message', observations );
...
function jump() {
  var t = d3.select(this);
		 	(function repeat() {
		  t = t.transition()
		  .call(zoomTo(belowsf, 15).event)
		  .transition()
		  .call(zoomTo(sf, 15).event)
		  .transition()
		  .call(zoomTo(reyes, 15).event)
		  .transition()
		  .call(zoomTo(funston, 15).event)
		  .each("end", repeat);
  })();
}
  ...
  projection
      ...
      .translate(zoom.translate());
		...
		var image = layer
				...
				.attr("class", "tile")
				.attr("src", function(d) { return "http://" + ["a", "b", "c", "d"][Math.random() * 4 | 0] + ".tiles.mapbox.com/v3/jhubley.c24f5987/" + d[2] + "/" + d[0] + "/" + d[1] + ".png"; })
